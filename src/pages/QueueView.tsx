...
import { useDownloadStore, selectDownloads, selectUpdateDownload } from "../store/useDownloadStore";
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
        ...
        <div className="flex gap-2">
          <button className="px-6 py-2.5 bg-white border-2 border-gray-100 hover:border-blue-600 hover:text-blue-600 text-gray-600 font-bold rounded-xl transition-all flex items-center gap-2">
            <Pause size={18} /> Pause All
          </button>
          <button className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-500/30 transition-all hover:-translate-y-0.5 flex items-center gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`text-sm font-black ${
                      ...
                      {download.status === 'error' ? 'FAILED' : `${download.progress}%`}
                    </span>
                  </div>
              ...
              <div className="relative h-2.5 bg-gray-100 rounded-full overflow-hidden mb-6">
                <div 
                  className={`absolute top-0 left-0 h-full transition-all duration-500 ${
                    download.status === 'error' ? 'bg-red-500' : 'bg-blue-600'
                  }`}
