...
class DownloadManager {
  async downloadFile(url2, savePath, onProgress) {
    try {
      const writer = require$$6.createWriteStream(savePath);
      const httpsAgent = process.env.NODE_ENV === "development" ? new https$2.Agent({ rejectUnauthorized: false }) : void 0;
...
function setupIPCHandlers() {
  ...
  ipcMain.handle("start-download", async (event, { url: url2, savePath }) => {
    try {
      ...
      console.log(`Starting download: ${url2} -> ${savePath}`);
      await downloadManager.downloadFile(url2, savePath, (progress) => {
