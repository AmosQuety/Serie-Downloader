...
const httpAdapter = isHttpAdapterSupported && function httpAdapter2(config) {
  return wrapAsync(async function dispatchHttpRequest(resolve2, reject, onDone) {
    ...
    if (config.transport) {
      ...
      transport = isHttpsRequest ? https$3 : require$$3$2;
    } else {
...
class DownloadManager {
  constructor() {
    ...
    __publicField(this, "INITIAL_RETRY_DELAY", 2e3);
  }
  // 2 seconds
  async downloadFile(url2, savePath, onProgress) {
    let lastError;
    for (let attempt = 1; attempt <= this.MAX_RETRIES; attempt++) {
      try {
        await this.executeDownload(url2, savePath, onProgress);
        return;
      } catch (error2) {
        lastError = error2;
        console.warn(`Download attempt ${attempt} failed for ${url2}: ${error2.message}`);
    ...
    throw new Error(`Download failed after ${this.MAX_RETRIES} attempts. Last error: ${lastError == null ? void 0 : lastError.message}`);
  }
  async executeDownload(url2, savePath, onProgress) {
    const writer = fs$1.createWriteStream(savePath);
    const httpsAgent = process.env.NODE_ENV === "development" ? new https$3.Agent({ rejectUnauthorized: false }) : void 0;
...
var truncate$2 = function truncate(getLength2, string, byteLength) {
...
var truncate2 = truncateUtf8Bytes;
var illegalRe = /[\/\?<>\\:\*\|"]/g;
...
function getOrganizedPath(basePath, meta, extension = ".mp4") {
  const seriesFolder = sanitize$1(meta.seriesTitle);
  let finalDir = path$1.join(basePath, seriesFolder);
  if (meta.season !== void 0) {
    const seasonFolder = `Season ${meta.season.toString().padStart(2, "0")}`;
    finalDir = path$1.join(finalDir, seasonFolder);
  }
  if (!fs$1.existsSync(finalDir)) {
    fs$1.mkdirSync(finalDir, { recursive: true });
  }
  let filename = "";
  if (meta.season !== void 0 && meta.episode !== void 0) {
    filename += `S${meta.season.toString().padStart(2, "0")}E${meta.episode.toString().padStart(2, "0")}`;
    if (meta.episodeTitle) {
      filename += ` - ${sanitize$1(meta.episodeTitle)}`;
    }
  } else {
    filename = sanitize$1(meta.episodeTitle || "download");
  }
  return path$1.join(finalDir, `${filename}${extension}`);
}
...
const error$r = {
  message: ({ params: { min: min2, max: max2 } }) => max2 === void 0 ? (0, codegen_1$C.str)`must contain at least ${min2} valid item(s)` : (0, codegen_1$C.str)`must contain at least ${min2} and no more than ${max2} valid item(s)`,
  params: ({ params: { min: min2, max: max2 } }) => max2 === void 0 ? (0, codegen_1$C._)`{minContains: ${min2}}` : (0, codegen_1$C._)`{minContains: ${min2}, maxContains: ${max2}}`
};
const def$O = {
  keyword: "contains",
  type: "array",
  ...
  code(cxt) {
    ...
    if ((0, util_1$C.alwaysValidSchema)(it, schema)) {
      let cond = (0, codegen_1$C._)`${len} >= ${min2}`;
      if (max2 !== void 0)
        cond = (0, codegen_1$C._)`${cond} && ${len} <= ${max2}`;
      cxt.pass(cond);
      return;
    }
    it.items = true;
    ...
    function checkLimits(count) {
      ...
      if (max2 === void 0) {
        gen.if((0, codegen_1$C._)`${count} >= ${min2}`, () => gen.assign(valid2, true).break());
      } else {
        gen.if((0, codegen_1$C._)`${count} > ${max2}`, () => gen.assign(valid2, false).break());
        if (min2 === 1)
          gen.assign(valid2, true);
        else
          gen.if((0, codegen_1$C._)`${count} >= ${min2}`, () => gen.assign(valid2, true));
      }
    }
  }
};
...
const logFile = path$1.join(app$1.getPath("userData"), "app.log");
function logInfo(message) {
  writeLog("INFO", message);
}
function logError(message, error2) {
  const errorMsg = error2 instanceof Error ? error2.stack : JSON.stringify(error2);
...
`;
  ...
  try {
    fs$1.appendFileSync(logFile, logEntry);
  } catch (err) {
...
if (!store.get("downloadPath")) {
  store.set("downloadPath", app$1.getPath("downloads"));
}
...
const dbPath = path$2.join(app$1.getPath("userData"), "downloads.db");
...
function setupIPCHandlers() {
  ipcMain$1.handle("show-save-dialog", async (_event, options = {}) => {
    try {
      ...
      console.error("File dialog error:", error2);
      return {
        canceled: true,
        filePath: null,
  ...
  ipcMain$1.handle("start-download", async (_event, { url: url2, savePath, metadata: metadata2 }) => {
    ...
    try {
      ...
      if (metadata2) {
        finalSavePath = getOrganizedPath(savePath, metadata2);
      }
      logInfo(`Starting download: ${url2} -> ${finalSavePath}`);
      await downloadManager.downloadFile(url2, finalSavePath, (progress) => {
      ...
      `).run(
        ...
        "completed",
        100,
        (metadata2 == null ? void 0 : metadata2.seriesTitle) || null,
        (metadata2 == null ? void 0 : metadata2.season) || null,
        (metadata2 == null ? void 0 : metadata2.episode) || null
      );
  ...
  ipcMain$1.handle("set-setting", (_event, { key, value }) => {
    store.set(key, value);
  });
  ...
  ipcMain$1.handle("get-download-history", () => {
    try {
      const stmt = db.prepare("SELECT * FROM download_history ORDER BY created_at DESC");
      return stmt.all();
    } catch (error2) {
      console.error("Failed to get history:", error2);
      return [];
    }
  });
  ipcMain$1.handle("save-download-record", (_event, record) => {
    try {
      const { url: url2, save_path, status, progress, title: title2, season, episode, thumbnail, description: description2, rating } = record;
      ...
      stmt.run(url2, save_path, status, progress, title2, season, episode, thumbnail, description2, rating);
      return { success: true };
    } catch (error2) {
      logError("Failed to save download record", error2);
      return {
        success: false,
        error: error2 instanceof Error ? error2.message : "Unknown error"
      };
